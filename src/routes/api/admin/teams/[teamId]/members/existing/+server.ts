...
import { SqliteTeamRepository, type ExistingPlayerInput } from '$lib/server/repositories/teams';
...
export const POST: RequestHandler = async (event) => {
  ...
  try {
    const record = await new SqliteTeamRepository(databaseFor(event)).addExistingPlayer(
      event.params.teamId,
      (await event.request.json()) as ExistingPlayerInput,
      user.id,
    );

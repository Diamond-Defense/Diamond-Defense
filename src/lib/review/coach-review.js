...
export function openCoachReview(attempt, fieldSource, bases = {}) {
  ...
  dialog.innerHTML = `<div class="attempt-review-heading"><div><span class="eyebrow">Coach review</span><h2 id="attemptReviewTitle"></h2><p class="attempt-identity"></p></div><button type="button" data-close>Close review</button></div>
    ...
    <aside class="attempt-review-details"><h3>Attempt result</h3><p data-result></p><dl><dt>Position score</dt><dd data-score></dd><dt>Tries used</dt><dd data-tries></dd><dt>Positioning time</dt><dd data-time></dd><dt>Practice context</dt><dd data-context></dd><dt>Sequence tries / time</dt><dd data-sequence-summary></dd><dt>Situation conditions</dt><dd data-conditions></dd></dl><h3 data-frame></h3><p data-check-score></p><p data-date></p><h3>Play sequence</h3><p data-picked></p><p data-expected></p><p data-sequence-result></p><p data-availability></p></aside></div>
  ...
  set('[data-result]', attempt.lifecycleStatus === 'incomplete' ? 'In progress' : attempt.outcome === 'abandoned' ? 'Not finished' : (attempt.outcome || (attempt.success === true ? 'passed' : attempt.success === false ? 'failed' : 'Not recorded')));
  ...
  set('[data-context]', [attempt.assignmentTitle || 'Free play', attempt.seasonName].filter(Boolean).join(' · '));
  const stages = Array.isArray(attempt.sequenceStages) ? attempt.sequenceStages : [];
  set('[data-sequence-summary]', stages.length ? stages.map((stage, index)=>`Stage ${stage.stage || index + 1}: ${stage.triesUsed ?? '—'} tries · ${stage.timeElapsed == null ? 'Time not recorded' : `${stage.timeElapsed}s`}`).join('; ') : attempt.phase === 2 ? `${attempt.triesUsed ?? '—'} tries · ${attempt.timeElapsed == null ? 'Time not recorded' : `${attempt.timeElapsed}s`}` : 'No sequence recorded');

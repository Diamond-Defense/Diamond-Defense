...
import { applyWranglerLogDefaults, PROJECT_WRANGLER_LOG_PATH, runNpm } from './lib/process.mjs';
import {
  PROJECT_ROOT,
  TEST_STATE,
...
export async function main(argumentsList = process.argv.slice(2)) {
  // Retain runtime diagnostics when the worker exits during a test run.
...
if (import.meta.url === invokedPath) {
  main().catch((error) => {
    console.error(`Test server failed during setup or execution: ${error.message}`);
    process.exitCode = 1;
  });
}
